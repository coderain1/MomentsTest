package com.thoughtworks.momentstest.adapter
...
import com.thoughtworks.momentstest.util.UrlUtils
import com.thoughtworks.momentstest.view.PhotoActivity
import com.thoughtworks.momentstest.widget.*

class MomentAdapter(val actionCallback: ActionCallback) :
    BaseQuickAdapter<TweetBean, BaseViewHolder>(R.layout.item_moments),
    ...
    override fun convert(helper: BaseViewHolder?, item: TweetBean?) {
        ...
        commentListView.setOnItemClickListener(object : CommentListView.OnItemClickListener {
             ...
             * 点击评论人名称，预留
             */
            override fun onItemUserClick(position: Int, user: UserBean?) {
            }
        })
        if (!hasComments) {
    ...
    interface ActionCallback {
         ...
         * @param tweet 点赞的动态
         */
        fun onLike(index: Int, tweet: TweetBean)
         ...
         * @param tweet 评论的动态
         */
        fun doComment(index: Int, tweet: TweetBean)
    }
}